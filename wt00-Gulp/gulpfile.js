...
gulp.task("mincss",function(){
    return gulp.src("src/css/*").pipe(gulp.dest("dist/css")).pipe(cssminfiy()).pipe(rename("main.min.css")).pipe(gulp.dest("dist/css"));
})

//lib copy 为了便于查阅源码 暂时不处理
...
gulp.task("minhtml",function(){
    return gulp.src("src/index.html").pipe(htmlmin({collapseWhitespace: true,minifyCSS:true,minifyJS:true,removeComments: true,removeStyleLinkTypeAttributes: true})).pipe(gulp.dest("dist"));
})

gulp.task("default",["mincss","minhtml", "copylib", "copyaudio","minimg"]);
